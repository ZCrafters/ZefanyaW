...
// --- CINEMATIC EMBER ENGINE ---
document.addEventListener('DOMContentLoaded', () => {
    // Inject the Cinematic UI wrappers into body
    const body = document.body;
    
    const cinematicLayer = document.createElement('div');
    cinematicLayer.className = 'global-cinematic-glow';
    body.insertBefore(cinematicLayer, body.firstChild);

    const fireLeft = document.createElement('div');
    fireLeft.className = 'fire-asset-left';
    body.insertBefore(fireLeft, body.firstChild);

    const fireRight = document.createElement('div');
    fireRight.className = 'fire-asset-right';
    body.insertBefore(fireRight, body.firstChild);

    // Apply magnetic glow to all major cards/sections
    const cards = document.querySelectorAll('.card-glass, .skill-card, .bento-card, .project-card, .section-box');
    cards.forEach(card => {
        card.classList.add('magnetic-glowing-element');
        card.classList.add('super-glow-container');
    });

    // Create Embers overlay
    const emberContainer = document.createElement('div');
    emberContainer.style.position = 'fixed';
    emberContainer.style.inset = '0';
    emberContainer.style.pointerEvents = 'none';
    emberContainer.style.zIndex = '-1';
    body.appendChild(emberContainer);
    ...
    function createEmber() {
        ...
        // Random color temperature (orange to red)
        const g = Math.floor(Math.random() * 100);
        ember.style.background = `rgb(255, ${g}, 0)`;
    ...
    // Add cinematic text reveal to all H1 and H2 tags
    const headers = document.querySelectorAll('h1, h2');
    headers.forEach(h => {
        if (!h.classList.contains('cinematic-text-reveal')) {
            h.classList.add('cinematic-text-reveal');
        }
    });
});
